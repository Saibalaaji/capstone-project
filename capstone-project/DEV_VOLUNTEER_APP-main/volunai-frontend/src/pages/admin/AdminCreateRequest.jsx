...
import { CirclePlus, Brain, Target, MapPin, Zap, Sparkles, CircleCheck } from 'lucide-react';
...
export default function AdminCreateRequest({ volunteers = [], onCreated, showToast }) {
    ...
    const handleAssign = async (requestId, volunteerId, name) => {
        ...
        try {
            ...
            setForm({ requesterName:'', requesterContact:'', location:'', lat: null, lng: null, serviceType:'', description:'', urgencyLevel:'MEDIUM' });
            ...
            <div className="grid-2" style={{ alignItems: 'start' }}>
                ...
                <form onSubmit={handleSubmit}>
                    <Card style={{ marginBottom: 20 }}>
                        <div className="card-title"><CirclePlus size={14} style={{ color: 'var(--coral)' }} /> Requester Info</div>
                    ...
                    <Card style={{ marginBottom: 20 }}>
                        ...
                        <div className="grid-2" style={{ gap: 16 }}>
                            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                                <label className="form-label">Location Coordinates <span className="required">*</span></label>
                                <LocationPickerMap 
                                    value={{ lat: form.lat, lng: form.lng, address: form.location }} 
                                    onChange={(loc) => setForm(f => ({ ...f, lat: loc.lat, lng: loc.lng, location: loc.address || `${loc.lat.toFixed(2)}, ${loc.lng.toFixed(2)}` }))} 
                                    height={200}
                                />
                                {!form.lat && <div style={{ fontSize: 11, color: 'var(--coral)', marginTop: 4 }}>* Please tap the map to pin the exact location.</div>}
                            </div>
                            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                                    ...
                                    return (
                                        <div key={match.volunteer.id} style={{
                                            ...
                                            <Button variant={idx === 0 ? 'primary' : 'outline'} size="sm"
                                                ...
                                                disabled={match.volunteer.availabilityStatus === 'BUSY'}
                                                icon={CircleCheck}
                                                style={{ width: '100%', justifyContent: 'center' }}>
                                                {match.volunteer.availabilityStatus === 'BUSY' ? 'Volunteer Busy' : `Assign ${match.volunteer.name.split(' ')[0]}`}
                                            </Button>
                                        </div>
                                    );
                                })}
