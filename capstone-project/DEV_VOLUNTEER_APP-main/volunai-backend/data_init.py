...
def init_sample_data():
    ...
    if not User.query.filter_by(email="admin@cvas.com").first():
        admin = User(name="Admin User", email="admin@cvas.com", contact_number="555-0001", location="New York", role="admin", is_verified=True, verification_status="VERIFIED")
    ...
    if not User.query.filter_by(email="user@cvas.com").first():
        user1 = User(name="John Doe", email="user@cvas.com", contact_number="555-0002", location="Brooklyn", role="user", is_verified=True, verification_status="VERIFIED")
        user1.set_password("user123")
        db.session.add(user1)
        db.session.commit()

    if not User.query.filter_by(email="volunteer@cvas.com").first():
        vol_user = User(name="Alex Volunteer", email="volunteer@cvas.com", contact_number="555-0003", location="Manhattan", role="volunteer", is_verified=True, verification_status="VERIFIED")
        vol_user.set_password("volunteer123")
        db.session.add(vol_user)
        db.session.commit()
        # Create linked volunteer profile
        if not Volunteer.query.filter_by(email="volunteer@cvas.com").first():
            vol_profile = Volunteer(
                name="Alex Volunteer", email="volunteer@cvas.com", phone="555-0003",
                location="Manhattan", rating=4.5, active=True,
                availability_status="AVAILABLE", is_verified=True, verification_status="VERIFIED"
            )
            vol_profile.set_service_types(["Medical Assistance", "Transportation", "Companionship"])
            vol_profile.set_available_days(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"])
            db.session.add(vol_profile)
            db.session.commit()
            print("[OK] Default volunteer created: volunteer@cvas.com / volunteer123")
